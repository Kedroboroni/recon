...
from events import getCoordinateRequest, transformationCoordinate, transformationDecimalOnDegries, filingTableCoordinate, writeCoordinateToExcel, openFile, recordingGeografyСoordinates
...
class MainWindow(QMainWindow):
    ...
    def mainWindow(self):
        ...
        LabelInfo = Lable(centralWidget, x = 4, y = 4, w = 916, h = 482, text = info)
        ...
        ButtonTransformDG.clicked.connect(lambda: recordingGeografyСoordinates(LineEditPath.text(), getInfo = LabelError))
        ButtonParse = Button(GroupBoxUseCastomFuncion, text = "Найти коардинаты объекта по названию", x = 612, y = 4, w = 300, h = 22)
        ButtonParse.clicked.connect(lambda: filingTableCoordinate(LineEditPath.text(), apiKey, getInfo = LabelError))
        ...
        ButtonChangeFile.clicked.connect(lambda: openFile(LineEditPath))
        LineEditPath= LineEdit(GroupBoxChangeFile, x = 106, y = 4, w = 449, h = 22) #Создали строку ввода (вводится автоматически при выобре пути, через кнопку Change или можно ввести вручную и нажать кнопуку Open)

        GroupBoxChangeFolder = GroupBox(centralWidget, x = 20, y = 524, w = 661, h = 30) #Создали коробку для размещения опций выбора источника потока/изображения/видео
        ButtonChangeFolder = Button(GroupBoxChangeFile, text = "Выбрать", x = 4, y = 4, w = 96, h = 22) #Создали кнопку выбора
        ButtonChangeFolder.clicked.connect(lambda: openFile(LineEditPath))
        LineEditPath= LineEdit(GroupBoxChangeFile, x = 106, y = 4, w = 449, h = 22)
